<template>
  <div>
    ...
    <group>
      <x-input title="用户名" type="text" placeholder="必填" v-model="loginName" label-width='80' text-align='left'></x-input>      
      <x-input title="密码" type="password" placeholder="必填" v-model="password" label-width='80' text-align='left'></x-input>
    </group>
    <x-button :text="submitBtnText" :disabled="submitStatus" :show-loading="submitStatus" @click.native="login" type="primary"></x-button>
  </div>
</template>

<script>
import { Cell, Group, Badge, Divider, XInput, XButton } from 'vux'
